...
// internal/runtime/goospkg source code to use. The default is obviously
...
const goospkg = "internal/runtime/goospkg"
...
func ResolveImport(moduleLoader *modload.Loader, imp string) (newPath, dir string, ok bool) {
	if !str.HasPathPrefix(imp, goospkg) || cfg.Goos != "tamago" {
		return "", "", false
	}
